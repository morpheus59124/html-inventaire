...
def is_register(mail):
    """
    Fonction qui va chercher dans la base de donnée le mail de l'utilisateur
    : param mail (str) : Le mail de l'utilisateur
    : return (bool) : Existe ou non
    """
    connexion = sqlite3.connect("./BDD/bdd.db")
    curseur = connexion.cursor()

    curseur.execute("SELECT role FROM user WHERE mail = ?", (mail,))
    reponse = curseur.fetchone()
    connexion.close()
    if reponse != None:
        return True
